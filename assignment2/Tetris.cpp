...
void Tetris::init(int **setOfBlockArrays, int nTypes, int nDegrees)
{
  ...
  for (int t = 0; t < numTypes; t++)
  {
    ...
    for (idx = 0; array[idx] != -1; idx++)
      ; // find the element of -1 in array[]
    for (size = 0; size * size < idx; size++)
      ; // comupte the square root of idx
...
{
  // Utils
  // cout << "Tetris::accept(" << key << ") called" << endl;
  // cout << "Tetris:: accept Current state: " << stateToString(state) << endl;
  // ===
  ...
  else if (state == TetrisState::Running)
  {
    ...
    case ' ':
      ...
      break;
    default:
      cout << "Tetris::accept: wrong key input" << endl;
    }
...
string stateToString(TetrisState state)
{
  switch (static_cast<int>(state))
  {
  case static_cast<int>(TetrisState::NewBlock):
    return "NewBlock";
  case static_cast<int>(TetrisState::Running):
    return "Running";
  case static_cast<int>(TetrisState::Finished):
    return "Finished";
  default:
    return "Unknown";
  }
